...
// AudioContext is created lazily inside fadeIn() to satisfy browser autoplay policy.
// Browsers block AudioContext construction before a user gesture — deferring fixes the
// "AudioContext was not allowed to start" console violation.
...
export function createIntroAudio(): IntroAudio | null {
  ...
  // All audio nodes are lazily initialised on first fadeIn() call
  ...
  return {
    fadeIn() {
      // AudioContext is created here — after the page has committed its first
      // frame, which satisfies the browser autoplay policy requirement.
      boot();
    ...
    pulse() {
      if (!ctx || !master || !osc1 || ctx.state !== 'running') return;
    ...
    resolve() {
      if (!ctx || !master || !osc1 || !osc2 || !osc3 || ctx.state !== 'running') return;
      const t = ctx.currentTime;
